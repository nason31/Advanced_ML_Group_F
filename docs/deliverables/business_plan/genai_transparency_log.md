...
Team Member: Leticia
Tool Used: Claude Code (claude-sonnet-4-6)
Task: Full Technical Architecture Review of all codebase files and fixing 4 identified bugs before presentation day.
AI Contribution: Claude performed a fresh read of every production file (app/main.py, app/components/briefing_card.py, app/components/audit_trail.py, src/recommendations/engine.py, src/recommendations/summarize.py, src/forecast/serve.py, src/rag/retriever.py, src/llm/reasoner.py, src/llm/prompts.py, src/llm/guard.py, src/llm/qa.py, src/data/product_names.py) and reported findings layer by layer.
  Review findings (no code changes):
  - Data/Forecast/RAG/Summarize layers: all clean. delta_pct fraction vs percent convention correctly documented and handled. ONNX DefaultEmbeddingFunction solid. lru_cache usage correct throughout.
  - guard.py: identified a false-positive path in the numeric check. The SYSTEM_PROMPT instructs "action first, then evidence", so for MARKDOWN recs the first percentage in the text is often the recommended discount (e.g. 20%) rather than the forecast delta (e.g. -5%). _extract_first_pct was grabbing the discount percentage, producing ratio = 20/5 = 4.0 > 2.0, flagging a valid recommendation as a hallucination.
  - engine.py: identified a crash risk - executor.map re-raises the first exception from any thread, so one failed Claude API call (rate limit, timeout) kills all 9 recommendations and triggers st.error() with zero recs shown.
  - main.py: _extract_sku called 3 times per rec in the active_products dict comprehension; if it returned a falsy value, get_product_name would receive bad input. Also noted _extract_sku returns "-" sentinel (not None) so crash path requires None explicitly.
  - qa.py: dead code in the product name matching condition - the second branch (q_upper in name.upper()) could never be true since a full question sentence is never a substring of a short product name.
  Code fixes (2 commits, 4 files):
  (1) b14beb6 fix(guard+engine): numeric check false positives and partial failure tolerance
      - src/llm/guard.py: rewrote _extract_first_pct to search for a signed percentage first ([+-]X%) before falling back to unsigned. Delta citations are always signed per SYSTEM_PROMPT ("repeat the exact percentage from the data, e.g. +419.6%"); action suggestions ("reduce by 20%") are unsigned. This keeps the numeric check active for every recommendation while reducing false positives. Verified on both the false-positive case (20% markdown, -5% delta -> now PASS) and a real hallucination (+999% vs +20% delta -> still FLAGGED).
      - src/recommendations/engine.py: wrapped _process_seed body in try/except. A single failed Claude API call now returns a flagged placeholder Rec with the error message rather than propagating through executor.map and crashing the entire pipeline. Remaining recs are unaffected.
  (2) 49897a2 fix(main+qa): guard None sku in active_products, remove dead condition
      - app/main.py: replaced dict comprehension with explicit loop; extracts sku once per rec and skips with if sku: guard before calling get_product_name/get_dept_name. Prevents potential crash when error-placeholder recs (no parseable SKU in text) are present.
      - src/llm/qa.py: removed dead branch from product name matching condition (q_upper in name.upper()), keeping only the correct direction (name.upper() in q_upper).
  Test suite: 19/19 passing after both commits. Both commits pushed to main.
Human Review: Questioned whether fix 2 (guard.py) was safer to skip entirely rather than add special-casing. Claude explained the fix does not skip the check - it makes the check target the right number (signed delta citation) instead of the wrong number (unsigned action suggestion). Accepted after understanding the reasoning. Confirmed fix 1 (engine.py try/except) is the correct pattern for partial failure tolerance in executor.map. Reviewed all diffs before authorising each commit.
...
Team Member: Leticia
Tool Used: Claude Code (claude-sonnet-4-6)
Task: Pre-presentation stress-testing via six structured evaluation prompts, followed by creation of a prioritised improvement checklist.
AI Contribution: Claude was given the full business plan PDF (MerchAI_Business_Plan.pdf, v1.3) and the complete source code. Six evaluation prompts were run in sequence, each from a different expert persona:
  (1) VC + business school professor: evaluated market opportunity, value proposition, GTM strategy, revenue model, and commercial feasibility. Identified three structural problems: the feedback-loop moat is circular (requires customers to build, customers require the moat), M5 data is not real retailer data, and the 0.5% ROI claim is borrowed from enterprise deployments with no MerchAI-specific evidence.
  (2) AI infrastructure + startup finance expert: evaluated unit economics against the actual pipeline. Found that token count is underestimated (~700 cited vs. ~1,200 realistic), the ChromaDB self-hosting claim is inconsistent with Streamlit Cloud's stateless architecture, the 91% gross margin excludes implementation and customer success costs (realistic figure 70-78%), and prompt caching is claimed but not implemented in reasoner.py.
  (3) LLM-as-a-Judge (wrapper risk): evaluated whether MerchAI has a defensible moat or is a thin Claude wrapper. Verdict: not a pure wrapper (the LightGBM layer produces the signal independently of the LLM), but not yet defensible (the audit trail moat is currently a session-state CSV with zero customer data; the M5 RAG corpus is public; the 54-phrase guard is trivially replicable). Identified SAP/Oracle/Salesforce as the real competitive threat, absent from the competitive landscape.
  (4) Full panel grading: scored all four rubric criteria. Commercial Innovation 7.5/10, Technical Execution 8.0/10, Defensibility 6.0/10, Presentation Clarity 8.0/10. Composite 7.4/10. Identified five improvements with highest grade leverage.
  (5) Senior AI product engineer code review: read every source file in the repository. Confirmed the pipeline architecture is sound and the parallelism, lru_cache, singleton client, and error handling are correctly implemented. Found three demo-day risks: prompt caching claimed but absent from reasoner.py; _extract_sku uses regex on LLM text (fragile); audit trail writes to an ephemeral Streamlit Cloud filesystem. Also identified that the cyclic date-to-day mapping in serve.py means forecasts are deterministic and non-real-time, and that SNAP references in the RAG corpus are US-specific and inapplicable to DACH/Iberia.
  (6) AI safety reviewer: evaluated hallucination, privacy, bias, overreliance, and transparency risks. Found guard blind spots (product name hallucinations, causal attribution hallucinations, non-percentage numbers unchecked). Found the AI disclaimer renders at 0.75em in light grey below the Accept/Reject buttons - effectively invisible and likely non-compliant with EU AI Act transparency requirements. Found the M5 dataset encodes systematic US bias (SNAP, Thanksgiving, US holiday seasonality) that will produce wrong recommendations if applied to European retail data without retraining.
  Output: docs/presentation_critique.md - a structured improvement checklist covering all six evaluations, triaged for one-week execution. Initial draft contained ~42 items across P1/P2/P3. After triage, 13 items were dropped as premature or zero-visibility to judges (including third guard layer, unit cap, store_id scoping, per-store API logging, accept-rate warning, outcome logging, and responsible use clause). Final checklist restructured into four executable blocks: Block 1 (11 business plan text fixes, Alex), Block 2 (6 small code fixes, Leticia + Justus), Block 3 (4 presentation/slides changes, Marie + Alex), Block 4 (4 demo day talking points, Marie). Projected composite score after fixes: 8.4/10.
Human Review: Reviewed all six evaluation outputs and confirmed triage decisions. Agreed to drop 13 items as out of scope for a one-week student timeline. Confirmed the four-block structure maps cleanly to team owners. The checklist structure, priority ordering, and triage rationale were human decisions made by Leticia after reviewing Claude's recommendations.
...
Team Member: Leticia
Tool Used: Claude Code (claude-sonnet-4-6)
Task: Implementing all Block 2 code fixes from the presentation critique checklist, debugging live app issues post-deployment, and hardening the hallucination guard.
AI Contribution: Claude implemented 6 Block 2 fixes across 5 files (commits 71ae3b9 through 7d5b951):
  (1) src/llm/reasoner.py: added retry loop with exponential backoff (up to 3 attempts) for RateLimitError and 5xx API errors. Added prompt caching via cache_control: ephemeral on the system message to reduce token costs on repeated calls within a session.
  (2) src/recommendations/engine.py: updated Rec dataclass with context_docs field (field(default_factory=list)) and sku field. Added _compute_confidence() mapping delta magnitude to "Strong" (>100%) / "Moderate" (>30%) / "Weak" (<30%). Capped ThreadPoolExecutor at max_workers=3 to avoid rate limit bursts. Updated _process_seed to populate sku and context_docs on each Rec.
  (3) app/components/briefing_card.py: added Weak Signal two-click confirmation dialog (st.session_state confirm_pending key). Added Context Sources section inside Details expander showing RAG docs. Moved AI disclaimer from per-row to single sidebar caption. Renamed column from "Confidence" to "Signal", "Action" to "Rec. Action". Added defensive getattr(rec, "sku", None) fallback for schema migration.
  (4) app/main.py: added _PIPELINE_VERSION = "v6" cache-busting constant. Added guard_flagged field to audit CSV. Replaced AI disclaimer from per-table note to sidebar caption. Added st.info demo banner (single sentence).
  (5) src/llm/guard.py: complete rewrite of numeric check. Replaced _extract_first_pct (fragile, grabbed first %-sign in text) with _actual_delta_cited presence-check: scans every percentage in the text, returns True if any is within 10% relative tolerance of the actual delta. This is order-independent and immune to RAG context percentages (e.g. "+15% threshold" or "+19.9% YoY") that Claude also mentions but are far from the SKU-level delta.
  Post-deployment debugging (5 additional commits):
  - AttributeError on rec.sku: old Recs in session state had no sku field. Fixed with getattr(rec, "sku", None) fallback in briefing_card.py.
  - Guard false positives (3 iterations): initial fix (closest-to-actual logic) still failed because the RAG context numbers happened to be closer to the actual delta in some cases. Final fix: moved guard execution from pipeline time (baked into Rec at generation) to render time (re-run from current guard.py on every page render). This makes the display immune to any stale guard results in session state - the UI always uses the current guard logic regardless of when the Rec was generated.
  - Nested expander crash (StreamlitAPIException): "Context Sources" expander was nested inside "Details" expander. Streamlit does not allow nested expanders. Fixed by replacing inner expander with a styled markdown header + captions.
  - Badge format fixes: removed "~" prefix from restock and markdown badges; kept only for promote revenue estimates. Confirmed promote with price: "~+€X rev", restock: "+X units", markdown with price: "↓ €X.XX", markdown without price: "-X% price".
Human Review: Reviewed every fix in the live Streamlit Cloud app at each iteration. Identified false-positive guard flags visually (screenshot review) and directed the guard rewrite approach ("not with first/last etc. - the text will always name numbers in different order"). Confirmed the render-time guard architecture as the correct fix after the presence-check alone was not sufficient. Confirmed badge format preferences. Reviewed all diffs before each push.
...
Team Member: Leticia
Tool Used: Claude Code (claude-sonnet-4-6)
Task: Applied a "senior AI product engineer" evaluation prompt to the full codebase, then implemented two fixes surfaced by the review: extending the hallucination guard to the Q&A path, and fixing the audit trail guard display.
AI Contribution: Claude read every production file and delivered a structured technical evaluation covering: frontend-backend connectivity, AI necessity, deployability risks, missing technical details, and grade-level risks. Key findings: (1) Q&A path had no hallucination guard while the briefing path did - a live demo risk if a judge asked a tricky question and got a wrong confident answer; (2) audit trail stored guard_flagged in CSV but never rendered it in the UI - the moat argument was invisible precisely where it needed to be visible. Three commits (653d076, e2794e1, 7523747) across 3 files:
  (1) 653d076 feat(guard): extend hallucination guard to Q&A and audit trail
      - src/llm/qa.py: answer_question now tracks matched_direction when a SKU is resolved from the question (both the product-name and SKU-code match paths). After the Claude response, runs guard.check() with delta_pct=None so only the intent check fires. Q&A prompts intentionally translate percentages to plain language ("about 5 times normal sales"), so the numeric % regex check would always false-positive; intent contradiction (saying "discount this" on an up-trending product) is the real risk. Return type changed from str to dict{answer, flagged, flag_reason}.
      - app/main.py: call site unpacks dict result; flagged and flag_reason stored in chat history entry; rendering loop adds st.warning() inside the assistant chat bubble when flagged, so the guard is visible in real-time during the demo.
      - app/components/audit_trail.py: each entry now shows a colour-coded badge (amber ⚠ flagged / green ✓ clean) derived from the guard_flagged CSV field. Added _is_flagged() helper to handle both bool (session state) and "True"/"False" string (CSV reload) representations.
  (2) e2794e1 fix(audit-trail): escape LLM text to prevent markdown breaking HTML layout
      - Initial attempt used html.escape() alone. html.escape() only neutralises HTML special chars (<, >, &) and does not strip markdown symbols (##, **, newlines). Streamlit parses markdown before rendering HTML, so ## headers and ** bold markers in the rec.text field still broke the layout even with html.escape().
  (3) 7523747 fix(audit-trail): strip markdown symbols before embedding in HTML
