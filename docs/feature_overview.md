...
| 🟢 **Promote This module** | Identifies SKUs forecast >+50% above 28-day baseline - strong momentum worth amplifying. Recommends channel action (end-cap, flyer, BOGO) via Claude. | delta_pct threshold logic, LLM channel reasoning | Commercial innovation |
| 🟢 **RAG knowledge base** | Retrieves relevant M5-derived context (SNAP lift, event effects, YoY trends, price elasticity) before each LLM call. Grounds output in real data. | ChromaDB, all-MiniLM-L6-v2 embeddings, 382 M5 context docs | Technical execution, Defensibility |
...
| 🟢 **ACTION column** | Each briefing card shows a colour-coded action badge (MARKDOWN / RESTOCK / PROMOTE) so managers can scan priorities at a glance without reading the full reasoning. | Price-action badge logic in recommendations engine, Streamlit UI | Technical execution, Presentation |
| 🟢 **Impact column** | Deterministic revenue and units estimate shown on every card (e.g. "+€1,200 / +240 units"). Gives managers a concrete expected outcome before they accept or reject. | delta_pct x baseline revenue/units formula, per-card display | Commercial innovation, Presentation |
