...
| 🟢 **Hallucination guard** | Every LLM claim is fact-checked against forecast data before reaching the manager. Intent check (54 retail action phrases) + numeric check (cited % vs actual delta). Mismatches are flagged and surfaced to the manager with full transparency - visible on every briefing card. | AI Safety (Week 5), output verification layer | Defensibility & Safety |
